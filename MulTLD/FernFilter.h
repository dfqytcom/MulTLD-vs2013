...
void FernFilter::saveToStream(std::ofstream & outputStream) const
{
  ...
  // 4. minVariance
  outputStream.write((char*)&ivMinVariances[1], ivNumObjects*sizeof(float));
//  outputStream.write(ivMinVariances.begin(), ivNumObjects*sizeof(float));
...
FernFilter FernFilter::loadFromStream(std::ifstream & inputStream)
{
  ...
  inputStream.read((char*)&minVariances[1], numObjects * sizeof(float));
